        ...
        for (int i = 0; i <= nStacks; i++) { // 0 1 2 ... n
            ...
            for (int j = 0; j <= nSlices; j++) {
                ...
                vertices[vertexIndex] =
                        ((float) torusRadius * cos(u)) + ((float) innerRadius * cos(v) * cos(u) + (float) 0);  // x
                vertices[vertexIndex + 1] =
                        ((float) torusRadius * sin(u)) + ((float) innerRadius * cos(v) * sin(u) + (float) 0);  // y
                vertices[vertexIndex + 2] = ((float) torusRadius * (float) 0) +
                                            ((float) innerRadius * cos(v) * (float) 0 +
                                             (float) innerRadius * sin(v));   // z
...
//		    glm::vec4 TESTtangente(-1.0f, 0.0f, 0.0f, 0.0f);  //Zum testen
                ...
                rotateX = glm::rotate(rotateX, (2.f * (float) PI / (float) 4),
                                      glm::vec3(1, 0, 0)); //rotiert fest um 90 Grad

                glm::mat4 rotateZ = glm::mat3(1.0f);
                rotateZ = glm::rotate(rotateZ, (2.f * (float) PI / (float) nStacks * (float) i),
                                      glm::vec3(0, 0, 1));  //rotiert  Z-Achse um u
        ...
        core->addAttributeData(OGLConstants::VERTEX.location, vertices, 3 * nVertices * sizeof(GLfloat), 3,
                               GL_STATIC_DRAW);
        /*Normalen-Array: Definieren die Ausrichtung der Vertices (Reflexionsberechnung/Lichtberechnung/ Helligkeit)*/
        core->addAttributeData(OGLConstants::NORMAL.location, normals, 3 * nVertices * sizeof(GLfloat), 3,
                               GL_STATIC_DRAW);
        /*Tangenten-Array und Binormalen-Array bilden mit den Normalen ein Koordinatensystem pro Vertex*/
        core->addAttributeData(OGLConstants::TANGENT.location, tangents, 3 * nVertices * sizeof(GLfloat), 3,
                               GL_STATIC_DRAW);
        /*Binormalen-Array*/
        core->addAttributeData(OGLConstants::BINORMAL.location, binormals, 3 * nVertices * sizeof(GLfloat), 3,
                               GL_STATIC_DRAW);
        /*Definiert Texture-Elements, Texels: Fuellt eine Bitmap, auf die Texturen "geklebt" werden*/
        core->addAttributeData(OGLConstants::TEX_COORD_0.location, texCoords, 2 * nVertices * sizeof(GLfloat), 2,
                               GL_STATIC_DRAW);
